...
class ResNet(nn.Module):
    ...
    def __init__(
        self, block, layers, num_classes=10, bit=32, in_channels=1, img_size=28
    ):
...
class Spiking_BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.part1 = Spiking_Oneway(
            conv3x3(inplanes, planes, stride), norm_layer(planes), IF(), self.T, k
        )

        self.part2 = Spiking_Twoways(
            conv3x3(planes, planes), norm_layer(planes), IF(), downsample, self.T, k
        )
...
class S_ResNet(nn.Module):
    ...
    def __init__(
        self, block, layers, num_classes=10, bit=4, in_channels=1, img_size=28, k=1
    ):
...
def resnet18_quickdraw(spike=False, num_classes=345, **kwargs):
    if spike:
        return S_ResNet(
            Spiking_BasicBlock,
            [3, 3, 2],
