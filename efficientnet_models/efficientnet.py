...
class FusedMBConvBlock(nn.Module):
    def __init__(
        ...
        self.part1 = Oneway(
            nn.Conv2d(
                in_channels, mid_channels, 3, stride=stride, padding=1, bias=False
            ),
...
class EfficientNetV2Net(nn.Module):
    ...
    def __init__(self, num_classes=345, bit=32, in_channels=1, img_size=28):
        ...
        self.layer1 = self._make_stage(C[0], C[1], self.STAGE_BLOCKS[0], stride=1)

        self.layer2 = self._make_stage(C[1], C[2], self.STAGE_BLOCKS[1], stride=2)

        self.layer3 = self._make_stage(C[2], C[3], self.STAGE_BLOCKS[2], stride=2)
...
class Spiking_FusedMBConvBlock(nn.Module):
    def __init__(
        self,
        in_channels,
        out_channels,
...
class S_EfficientNetV2Net(nn.Module):
    STAGE_CHANNELS = EfficientNetV2Net.STAGE_CHANNELS
    STAGE_BLOCKS = EfficientNetV2Net.STAGE_BLOCKS
    EXPAND_RATIO = EfficientNetV2Net.EXPAND_RATIO
...
def efficientnetv2_quickdraw(spike=False, num_classes=345, **kwargs):
    if spike:
        return S_EfficientNetV2Net(
            num_classes=num_classes, in_channels=1, img_size=28, **kwargs
        )
    else:
        return EfficientNetV2Net(
            num_classes=num_classes, in_channels=1, img_size=28, **kwargs
        )
