...
from efficientnet_models.quant_layer import QuantReLU
...
def parallel_param_normalization(snn, k=1):
    ...
    print("=> Parallel parameter normalisation applied.")
...
parser = argparse.ArgumentParser(
    description="EfficientNetV2 SNN fine-tuning — QuickDraw"
)
parser.add_argument("--epochs", default=300, type=int, metavar="N")
parser.add_argument("-a", "--arch", default="efficientnetv2_quickdraw", metavar="ARCH")
...
parser.add_argument(
    "--bit", default=2, type=int, help="QAT bitwidth — determines T = 2^bit − 1"
)
parser.add_argument(
    "-k",
    "--k",
    default=3,
    type=int,
    help="max spike level for SMT-IF (k=1 = standard signed IF)",
)
parser.add_argument(
    "-n",
    "--num_epochs",
...
def main():
    ...
    print("=> Building EfficientNetV2 models (ANN, proxy ANN, SNN)...")
    ...
    model = efficientnet_models.efficientnetv2_quickdraw(
        num_classes=num_classes, bit=args.bit
    )
    net = efficientnet_models.efficientnetv2_quickdraw(
        num_classes=num_classes, bit=args.bit
    )
    snn = efficientnet_models.efficientnetv2_quickdraw(
    ...
    if args.init and os.path.isfile(args.init):
        print(f'\n{"="*60}')
        print(f"=> Loading ANN checkpoint from: {args.init}")
        ...
        print(f"        Run main_effnet.py first.")
        exit()
