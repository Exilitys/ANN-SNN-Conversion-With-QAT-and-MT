...
parser = argparse.ArgumentParser(description="AlexNet QAT training on QuickDraw")
...
parser.add_argument(
    "--init", default="", type=str, help="path to pre-trained floating-point weights"
)
parser.add_argument("-id", "--device", default="0", type=str)
parser.add_argument(
    "--bit", default=32, type=int, help="activation bit-width (32 = full precision)"
)
...
def train(trainloader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(trainloader):
        ...
        if i % args.print_freq == 0:
            eta_seconds = batch_time.avg * (len(trainloader) - i)
